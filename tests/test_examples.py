...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("examples.aws.main", {"cloud_provider": "aws", "cloud_region": "eu-north-1"}),
        (
            "examples.azure.main",
            {
...
async def test_provider_example_is_runnable(module_name, provider_fields, monkeypatch, capsys):
    monkeypatch.setenv("SERVICE_NAME", "example-api")
    monkeypatch.setenv("SERVICE_ENV", "test")
    monkeypatch.setenv("SERVICE_VERSION", "v0.1.0")
    ...
    monkeypatch.setenv("AZURE_RESOURCE_GROUP", "example-rg")
    ...
    for entry in (application, access):
        assert entry["service"] == "example-api"
    ...
    assert application["component"] == "health"
