...
- Before committing, run `just check` and `git diff --check`.
- Run `just package-check` when distribution contents or release metadata
  change.
...
- Run `just install`, `uv lock --check`, `just check`, `just package-check`, and
  `git diff --check`.
